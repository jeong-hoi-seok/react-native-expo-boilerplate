...
React Native·Expo 작업 시 **공식 문서를 우선** 확인합니다. 이 저장소의 `docs/`·`wiki/`는 프로젝트 규칙이고, API·동작·플랫폼 제약은 아래 공식 문서가 기준입니다.
...
- **UI 스타일링**은 [NativeWind Docs](https://www.nativewind.dev/docs/getting-started/installation)를 우선합니다. RN `Style` API는 NativeWind로 표현이 어려울 때만 보조로 봅니다.
...
| 라우팅·레이아웃 | [Expo Router](https://docs.expo.dev/router/introduction/) | `app/` 파일 기반 라우트, 탭·스택 |
| UI 스타일링 (`className`) | [NativeWind Docs](https://www.nativewind.dev/docs/getting-started/installation) | Tailwind 유틸, 테마, 다크 모드, `remapProps` |
...
1. 이 저장소 `agent.md` · 관련 `docs/` · `wiki/index.md` 확인
2. **Expo Docs**에서 SDK·도구·배포 관련 내용 검색
3. 스타일·레이아웃 작업은 **NativeWind Docs**에서 `className`·Tailwind 패턴 확인
...
- 스타일 구현 시 `StyleSheet`·inline `style` 대신 **NativeWind `className`**을 기본으로 제안합니다.
